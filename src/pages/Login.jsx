...
const Login = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (registeredDoctor) {
        ...
        if (doctorData.email === user.email || doctorData.googleId === user.uid) {
  ...
  const handleGoogleSignupSubmit = (additionalData) => {
    ...
    localStorage.setItem('doctor_registration', JSON.stringify(doctorData));
    localStorage.setItem('doctor_logged_in', 'true');
      ...
      <div className="max-w-md w-full">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-3xl shadow-2xl p-8 border border-gray-100">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-6 bg-gradient-to-r from-purple-500 to-purple-600 text-white py-4 rounded-2xl font-semibold text-lg hover:from-purple-600 hover:to-purple-700 transition-all shadow-lg hover:shadow-xl flex items-center justify-center space-x-2 disabled:opacity-50 disabled:cursor-not-allowed"
          >
